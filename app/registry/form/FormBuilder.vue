...
<template>
  <form class="grid grid-cols-1 md:grid-cols-3 gap-2" @submit="form.handleSubmit">
    <div v-for="field in fields" :key="field.name" class="space-y-2 flex flex-col items-start justify-start">
      <Label
        :for="field.name"
      ...
      <span v-if="getError(field.name)" class="text-sm text-red-500 font-medium">
        {{ getError(field.name) }}
      </span>
    </div>
